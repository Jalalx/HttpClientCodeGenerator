...
namespace HttpClientGenerator
{
    ...
    public class HttpClientCodeGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            {
                foreach (var info in syntaxReceiver.HttpServiceInfos)
                {
                    foreach (var group in info.Value.Methods.GroupBy(f => f.MethodSymbol.ContainingType))
                    {
                        var classSource = ProcessClass(group.Key, info.Value, group.ToList(), syntaxReceiver.ImportedNamespaces, context);
        ...
        private string ProcessClass(INamedTypeSymbol classSymbol, MarkedMethodSyntaxReceiver.HttpBaseSerivceInfo info, List<MarkedMethodSyntaxReceiver.MarkedMethod> methodSymbols, IEnumerable<INamespaceSymbol> namespaceSymbols, GeneratorExecutionContext _)
        {
            ...
            {
                ProcessMethod(source, method.MethodSymbol, method.HttpVerbAttributeSymbol, info.HttpClientSymbol);
            }
